...
dfs_linguistic_scale = {
    ...
    'EMI': {'O': (0.90, 0.10), 'P': (0.10, 0.90)},  # Exactly More Importance
}
...
def ml_expert_weighting(expert_data, cov_matrix=None):
    ...
    if cov_matrix is None:
        ...
        expert_data_normalized = (expert_data - min_vals) / range_vals
...
def step1_ml_expert_weighting():
    ...
    if data_option == "Use default example data":
        ...
        cov_matrix = np.array([
            ...
            [0.1153, 0.1237, 0.1441, 0.0890, 0.0480, 0.0508, -0.0148, 0.0720, 0.0410, 0.0250],
            [0.1250, 0.1250, 0.0890, 0.0833, 0.0417, 0.0250, 0.1319, 0.1042, 0.0243, 0.0741],
            [0.0570, 0.0820, 0.0480, 0.0417, 0.1319, 0.1042, 0.0243, 0.0741, 0.1354, 0.1417],
            [0.1184, 0.1289, 0.0508, 0.0250, 0.1042, 0.0938, 0.0556, 0.0750, 0.1222, 0.1157],
            [0.1338, 0.1322, -0.0148, 0.1319, 0.0243, 0.0556, 0.1222, 0.1157, -0.0365, 0.0208],
            [0.1155, 0.1302, 0.0720, 0.1042, 0.0741, 0.0750, 0.1157, 0.1516, 0.0139, 0.1056],
            [0.0066, 0.0332, 0.0410, 0.0243, 0.1354, 0.1222, -0.0365, 0.0139, 0.1056, 0.0139],
            [0.0526, 0.0828, 0.0250, 0.0741, 0.1417, 0.1157, 0.0208, 0.1056, 0.0139, 0.1375]
        ])
    ...
    if st.button("Compute Resilience Challenge Weights"):
        with st.spinner("Computing DFS-AHP weights..."):
            try:
                ...
                normalized_weights = [w/sum(defuzzified_weights) for w in defuzzified_weights] if sum(defuzzified_weights) > 0 else defuzzified_weights
...
def step5_results_summary():
    ...
    with col1:
        if st.button("Previous: MILP Optimization", key="step5_prev"):
            ...
            st.rerun()
    with col2:
        if st.button("Restart Analysis", key="step5_restart"):
            # Clear session state
